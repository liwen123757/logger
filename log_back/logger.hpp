...
namespace lwlog
{
    ...
    class LoggerManager{
    public:
        static LoggerManager &getInstance()//获取单例对象,懒加载模式
        {
    ...
    private:
        LoggerManager(){
            std::unique_ptr<LoggerBuilder> builder(new LocalBuilder());
    ...
    //设计一个全局日志器的建造者--子啊局部的基础上增加了一个功能：将日志器添加嗲单例对象中
    ...
    public:
        Logger::ptr build()
        {
            ...
            if(_logger_type==LoggerType::LOGGER_ASYNC)
            {
                logger=std::make_shared<AsyncLogger>(_logger_name,_limit_level,_formatter,AsyncType::ASYNC_SAFE);
            }
            else{
